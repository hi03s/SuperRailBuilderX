...
### 2026-09-06 ローカルCodex — 複線・分割再失敗と正式レール移動ツール

- `logs/latest.log`から、複線コピーの3本目が既設通常道床で埋まった区間のセクションコア候補を確保できず、`section_core_conflict`で全体をロールバックしていたことを特定した。セクションコアは既設通常道床を置換可能とし、既設コアは従来どおり保護するよう修正した。
- 通常道床の重複規則を変更し、経路途中は既設レールの所有情報を維持し、新設側の非コア終端道床だけを置換するようにした。セクションコアは既設通常道床を置換する。
- 線路分割は、生成されるどちらか一方が6 m以下ならクライアント候補とサーバー適用の両方で拒否するよう変更した。Undo復元を`BlockMarker.createRail`からbuilder1共通生成へ移し、分割・Undo・ロールバック時の削除対象と更新対象をクライアントへ通知して残像を除去するようにした。
- builder1のスナップ中3×3マーカーを実際の視線衝突位置へ表示するよう変更した。既設端勾配を保つ直線の後に0‰へ移る正円近似縦曲線を置き、自由端を地上1/16 mへ合わせるIキー操作を追加した。
- `SuperRailBuilderX_RailPositionTest`を正式な`SuperRailBuilderX_RailMover`へ改名し、モデル定義・クライアント・サーバースクリプト・資料を更新した。
- 生ログはコミットせず、原因判断に必要な行だけを`logs/rail-tools-retest-20260906-5.log`へ保存した。
- 検証: `pnpm gen`成功、`pnpm build`はcommon・kaizpatch・mc1710・mc1122で成功。変更ファイルのPrettier、JSON解析、`git diff --check`に成功。勾配-2°、半径1000 m、10.0625 mから4.0625 mの例で必要水平距離189.2726 m、2区間、終端高4.0625 mとなる静的数値確認に成功した。`pnpm format:check`は参照専用ツールキットとNGTOBuilder2サンプルの既知26ファイルだけで失敗した。
- 未実施: Minecraft実機での複線生成、分割・Undo、Iキー生成・走行、道床所有と重複レール走行。
- 実装コミット: `85e3572`（`origin/main`へ同期済み）。

新しい記録を上に追加します。詳細な議論がGitHub Issueにある場合は、要点とリンクだけを記載します。

### 2026-09-06 ローカルCodex — 複線Undo・短レール分割・RailPosition表示同期の修正

- 最新`logs/latest.log`では、複線生成・Undo成功後に削除済み`core:-197,4,357`を複製元として再送して`source_rail_changed`となり、別の再生成では`rail_endpoint_changed`となってロールバックしていた。Undo対象キーをクライアントの候補探索と端点吸着から除外し、該当するクライアント側論理レールも明示的に破棄するよう変更した。
- 曲率中心制限は近似半径の符号に依存せず、元レールとオフセット後の水平弦の内積がほぼゼロまたは反転する列から、全プレビュー・生成を打ち切る方式へ変更した。
- 線路分割ログでは繰り返し分割後に2.78 m / 4.81 m、5.31 m / 2.28 mの短区間生成と`invalid_split_position`を確認した。全長6 m以下をクライアント候補とKaizPatchXサーバー適用の両方で拒否するよう変更した。
- builder1の`selectCursorMarker`を、選択位置と同じ高さの水平3×3範囲へ展開した。
- RailPosition移動は地面の移動先へ通常レール高1/16 mを加え、別レール上ではRailMapの基準高へ合わせるよう変更した。builder1のモデル・テクスチャへ統一し、Pキーの水平0.1 mスナップを追加した。
- RailPosition再生成後の通常コアまたは全セクションコア座標をサーバーから通知し、クライアントでRailMap再構築・再描画・ブロック更新を行うようにした。共有端点の部分適用失敗時も、適用済みコアは更新する。
- 生ログはコミットせず、必要行だけ`logs/rail-tools-retest-20260906-4.log`へ保存した。引継ぎ帳の確認済み項目を削除し、優先確認事項を今回ゲーム内で確認が必要な内容だけへ整理した。
- 検証済み: 変更対象のPrettier、モデルJSON解析、`git diff --check`、`pnpm gen`、`pnpm build`（kaizpatch・mc1710・mc1122）。
- 未検証: 複線Undo後の再プレビュー・ゴースト除去と曲率中心制限、6 m境界と繰り返し分割、builder1の3×3マーカー、RailPositionの高さ・モデル・Pスナップ・連続端点移動後の即時描画。
- 実装コミット: https://github.com/hi03s/SuperRailBuilderX/commit/e40f361
...
### 2026-09-06 ローカルCodex — レールツール再検証結果の修正

- `logs/latest.log`を調査し、複線コピーの成功・Undo応答がクライアントで各3回処理されていること、Undo済みの`core:-180,4,341`を複製元として送信して`source_rail_changed`になっていることを確認した。必要部分だけを`logs/rail-tools-retest-20260906-2.log`へ抜粋し、生ログはコミット対象外とした。
- builder1は、自動分割対象の経路上にある未保護コアがKaizPatchXセクションコアだけの場合、新設側を`autoSplit=false`の単一通常レールとして生成し、既設セクションコアを道床で上書きしない経路を追加した。通常コアとの衝突と生成始点の占有は従来どおり安全停止する。
- 複線コピーを、レール選択後に空間を右クリックして複製位置を固定し、Enterで生成する段階操作へ変更した。左クリックは位置固定を1段階戻し、間隔・選択変更では固定を解除する。
- 選択済みレールのホバーを暗い水色に変更した。クライアント応答を要求中の1回だけ処理してUndo用選択スナップショットの上書きを防ぎ、Undo時は撤去経路全体をブロック更新して削除済みTileEntityの残留を防ぐ。
- 線路分割の距離パネル描画時に標準アルファブレンドと白色を設定し、透明部分が黒くなる問題を修正した。分割側にも応答の1回処理を適用した。
- 低速遷移不能は再現したが、ログには発生時刻・方向・速度がなく生成診断との対応は未確定。KaizPatchXの`RailTransitionResolver`が端点越え判定後に接続先探索する構造と低速時の座標拘束との相互作用が候補のため、次回は発生条件を時刻と対応させる。
- 検証済み: 変更対象のPrettier、`git diff --check`、`pnpm gen`、`pnpm build`（kaizpatch・mc1710・mc1122）。
- 未検証: ゲーム内でのチャンク境界交差、複線コピーの新操作・Undo選択復元・削除後同期、暗色ホバー、距離パネル透明描画、低速遷移。
...
### 2026-09-06 ローカルCodex — 内部コア衝突とレール平行移動の修正

- `logs/latest.log`を調査し、複線コピーとbuilder1の`section_core_conflict`は、端点の接続マーカーブロック`-228,4,416`ではなく、直前に生成された通常コアと次レールの内部セクションコア候補が`-228,4,415`で衝突したことが原因と特定した。64 m以下で生成始点が空いている場合は単一通常レールへ切り替えるようにし、必要箇所だけ`logs/rail-tools-retest-20260906-6.log`へ保存した。
- builder1はスナップ状態によらず実視線位置から通常マーカー候補を表示し、辺・端・角では8方向の結果を重複排除して全候補を描くよう変更した。向き決定後は自由点の所有ブロックとマーカーを同じ規則で更新する。
- 手にレールがないbuilder1生成では接続端点のRailPropertyを継承し、両端が既設レールなら始点側を優先する。Iキーの0‰地上高合わせは現在の地上高で延長できない場合に1ブロック上・下も探索する。
- 前回のレール移動は経路途中の全既設通常道床を保持したため、移動後レールが所有する道床を得られず`Rail not found > x:-176 z:542`になっていた。移動時は既設レールの両端から曲線長約2 m以内だけを保護し、他の重複通常道床は移動後レール側へ置き換える規則に戻した。既設コアは常に保護する。
- レール移動ツールへ、中央ホバーから1本の論理RailMapを選択して平行移動する段階操作を追加した。カーソルから平行距離を求め、Pで0.1 mスナップし、水平弦長比で水平・垂直アンカーを調整する。曲率中心越えを拒否し、移動先端部は0.5 m以内の別端点へ接続する。成功時は旧RailMapゴーストを除去し、失敗復元時も復元コアをクライアントへ通知する。
- 検証済み: 変更対象のPrettier、`git diff --check`、`pnpm gen`、`pnpm build`（kaizpatch・mc1710・mc1122）、生成JavaScript内の新API・通常レールフォールバック・端部道床保護を確認。
- 未検証: Minecraft実機での複線・builder1通常レールフォールバック、候補マーカー、モデル継承、Iキー±1ブロック探索、レール移動の道床所有・走行・平行移動・端点接続。
- 実装コミット: https://github.com/hi03s/SuperRailBuilderX/commit/7434eec
- 同期: `origin/main`へ同期済み。

### 2026-09-06 Web側Codex — モデル使い分け規則

- OpenAI公式資料を確認し、Work側とVS Code側の双方でモデル選択とモデル指定サブエージェントを利用できる一方、実行中の親エージェント自身が難易度に応じて別モデルへ自動移行する仕組みではないことを整理した。
- `AGENTS.md`へ、単純検索・ログ抽出・定型検証は軽量モデル、限定実装・探索はバランス型モデル、設計・破壊的変更・統合確認は親または高能力モデルとする基準を追加した。
- 委譲コンテキストと統合コストが節約分を上回る小作業ではサブエージェントを使わず、モデルが利用不可でも親モデルで継続する規則を追加した。

### 2026-09-06 ローカルCodex — 分割点衝突・モデル継承・レール移動Undo

- `logs/latest.log`を調査し、短い通常レールの分割失敗は前半終端道床と後半通常コアが共有分割点の同一ブロックを要求する`section_core_conflict`と特定した。KaizPatch側の制約ではなくツール側で解決可能なため、共有分割点として明示した保護中の非コア道床だけ所有座標を自身へ戻してコア化し、既設コアと分割点以外の保護道床は従来どおり置換しないよう修正した。
- 手にレールを持たないbuilder1の勾配生成では、最初の`rail->free`区間後に内部`free->free`区間が接続先RailPropertyを参照できず`hold_rail_item`でロールバックしていた。元の既設端点を全内部区間のモデル取得元として渡すよう修正した。既設端点ホバー時の予測表示は`snapCursorMarker`へ変更した。
- レール平行移動時に、移動元の両端へ接続している別レールを検出し、その接続端点も新しい端点へ連動移動するよう実装した。移動先の片側だけが別端点へスナップする場合は反対の自由端へ同じ補正量を加え、両側に接続先がある場合は両方の正確な接続を優先する。
- レール移動ツールへCtrl+Zを追加し、通常の端点移動、平行移動、連動移動した前後端点をサーバー側の1記録として逆順復元する。移動先・復元先にある接続レールを生成前に保護し、更新コアと旧RailMapの削除情報をクライアントへ同期する。対象が変更・撤去済みなら安全停止し、部分適用時は成功済み操作だけUndo可能にする。
- 生ログはコミット対象外とし、原因判断に必要な行だけを`logs/rail-tools-retest-20260906-7.log`へ匿名化・抜粋した。
- 検証済み: 対象ファイルのPrettier、`git diff --check`、`pnpm gen`、`pnpm build`（kaizpatch・mc1710・mc1122）。
- 未検証: Minecraft実機での共有分割点の生成・走行・Undo、builder1の勾配付きモデル継承と端点ホバー表示、レール移動の前後端点連動・片側/両側接続補正・Undo。
- 実装コミット: https://github.com/hi03s/SuperRailBuilderX/commit/5eea5844565ae16ae722c50068d50b9e6884e437
- 同期: `origin/main`へ同期済み。

### 2026-09-07 ローカルCodex — 専用アセット・分割表示・レール複数移動

- 開発者が追加したbuilder1・複線コピー・線路分割・レール移動の本体、ボタン、アイコンテクスチャを各モデルJSONへ適用し、編集用`button_template.xcf`も保存した。NGTOBuilder2由来のSampleモデル、JSON、スクリプト、テクスチャは削除した。
- 線路分割は分割後の両区間を3 m超にする制限へ戻し、全長6 m以下など有効な候補を持たないレールを赤く強調して選択できないようにした。実機ログでは共有分割点を含む分割が連続して成功し、今回の失敗は再現していない。
- レール移動は論理RailMapの複数選択・解除、同一平行距離での一括プレビュー・適用、一括Undo記録に対応した。選択中レール同士は前後端点連動の対象から除外し、同じ外部接続端点を重複して動かさない。
- Ctrl+Zは不正確だったクライアント側Undo可否フラグで遮断せず、必ずサーバーへ要求するよう変更した。適用・Undo後は更新された全物理コアの描画を更新し、撤去済み論理キーを候補探索から除外する。重なり道床でも正しいRailMapを拾いやすいよう中央ホバー探索を水平2ブロックへ拡張した。
- 生の`logs/latest.log`はコミット対象外とし、分割・レール移動の成功とUndo要求不在の判断に必要な行だけを`logs/rail-tools-retest-20260907-8.log`へ匿名化して保存した。
- 検証済み: 変更対象のPrettier、`git diff --check`、`pnpm gen`、`pnpm build`（kaizpatch・mc1710・mc1122）、配布物内の新規テクスチャ・モデル参照・生成JavaScriptを確認した。全体の`pnpm format:check`は参照専用ツールキット既存24ファイルの整形差異だけで失敗したため変更していない。
- 未検証: Minecraft実機での分割不可レール赤表示と3 m境界、複数レール平行移動、一括Undo、端部モデル・ホバー強調の即時同期。
- 実装コミット: `7850dc9`
- 同期: `origin/main`へ同期済み。
...
### 2026-09-07 ローカルCodex — レール移動クラッシュ・入力・選択修正

- `logs/latest.log`を調査し、平行・端点移動のUndo記録は作成されている一方、レール移動ツールのCtrl+Zだけクライアントのチャットにもサーバー要求にも到達していないことを確認した。レール移動だけ`InputManager#setOptionKey`が未設定だったため、左Ctrlを修飾キーとして登録した。
- 描画クラッシュは、移動先のレール高さ探索が未初期化セクションコアのnullな`getLogicalRailPositions()`を直接RailMap化したことが原因だった。高さ探索で非対応コアを除外し、compatの論理RailMap取得にも例外ガードを追加した。
- 複数平行移動の追加選択を、すでに選択したいずれかのレールと端点で接続する論理レールだけに制限した。サーバーでも対象全体が1つの接続グラフで、論理キーが重複していないことを適用前に検証する。
- 選択済みレールのホバー色を複線コピーと同じ暗い水色へ統一し、描画順で通常の水色に上書きされないよう修正した。サーバー応答待ち中は旧線形の強調・プレビューを描かないようにした。
- builder1のモデル表示名を`レール生成A`へ変更し、README、統合操作ガイド、同梱readme.txt、技術資料へ反映した。開発者更新の線路分割ボタンテクスチャも含めた。
- 生の`logs/latest.log`はコミット対象外とし、原因判断に必要な行だけを`logs/rail-mover-crash-undo-retest-20260907-9.log`へ匿名化して保存した。
- 検証済み: 対象ファイルのPrettier、`git diff --check`、`pnpm gen`、`pnpm zip`。全3ターゲットのビルドと`SuperRailBuilderX-alpha-0.1.0.zip`生成、生成JavaScript内の修飾キー・接続検証・クラッシュガードを確認した。
- 未検証: Minecraft実機でのレール移動Ctrl+Z、接続レール限定選択、暗色ホバー、ゴースト抑止、クラッシュ再発防止、レール生成Aの表示名。
- 実装コミット: `667481b`
- 同期: `origin/main`へ同期済み。

### 2026-09-07 ローカルCodex — カント整形・分岐生成ツール

- カント整形ツールを追加した。設計速度は初期100 km/h、上下キー1 km/h刻みの長押し対応・チャット非表示・`curvePanel`数字表示とし、Ctrl+左右で1067/1372/1435在来線/1435新幹線/1000モノレールを切り替える。均衡カント式へカント不足は加えず、SRB3準拠の最大105/150/150/200/110 mmと`asin(高さ/軌間)`で角度化する。通常/自動分割レールの端点複数選択、`cantPanel`表示、一括適用・Undo、サーバー再検証と物理コア同期を実装した。
- 分岐生成ツールを追加した。既設水平レールを約0.25 m候補で分割し、分割点から自由点/別の水平レール端点へのbuilder1方式プレビューを作る。新線方向側の既設半区間と新線を単純分岐へ合成し、反対半区間は通常レールで残す。P/Ctrl+Pスナップ、Oと左右キーの半径選択・固定、手持ち/分割元モデル、レール移動準拠の道床、生成前復元Undoを実装した。
- 勾配・縦曲線はRailMapの始端・中央・終端Pitchで、分岐はcompat型とRailMap取得で除外する。分割点・端点・論理キー・在線状態をサーバー側でも再検証し、生成例外時は残存半区間を撤去して元レールの復元を試みる。mc1710/mc1122は共通UIを生成するがワールド変更は`unsupported_target`で停止する。
- 検証済み: `git fetch origin --prune`で開始時`main`=`origin/main`=`b6c9a4f`、`pnpm gen`、`pnpm build`（common・kaizpatch・mc1710・mc1122）、対象Prettier、JSON解析、カント上限角度と計算値の数値確認、`git diff --check`。
- 未検証: Minecraft実機でのカント方向・自動分割レール同期・複数Undo、分岐生成・切替・道床・走行・接続先スナップ・モデル選択・Undo・拒否条件。新規2モデルは暫定でbuilder1の本体・ボタン・アイコンテクスチャを共用する。
- 実装コミット: `51f855d`
- 同期: `origin/main`へ同期済み。
...
### 2026-09-09 ローカルCodex — カント連続反映・端点分岐形状修正

- `logs/latest.log`ではカント適用・Undoがすべて成功して例外がなく、共有端点の片側だけを要求へ含める仕様上の不足と判断した。選択端点に接続する別レールをサーバーで再検証し、相手側の`cantEdge`へ反対符号を同時適用するよう修正した。Undoにも自動更新した接続相手を含める。
- 曲率接線のYaw増加が右曲線、RTMの正カントが右側を持ち上げる向きであることから、均衡カントの曲率符号を反転した。始終端のRailPosition表現に必要な相互反転は維持する。
- 端点起点の分岐生成が通常レールを外向きへ追加していた実装を廃止した。選択端点の元アンカー方向・長さを分岐根元、元レールの反対端を本線、新しい終点を側線とする単純分岐へ組み替え、信号・サブレール・モデルとUndo時の元レール復元を維持する。
- 生ログは追加せず、結果判断に必要な成功行だけを`logs/cant-branch-retest-20260909.log`へ匿名化して保存した。端点分岐の再試験用に根元・本線・側線の座標、Yaw、アンカー長を診断出力する。
- 検証済み: 対象Prettier、`git diff --check`、`pnpm gen`、`pnpm build`（kaizpatch・mc1710・mc1122）。
- 未検証: Minecraft実機での共有端点両側カント・左右符号・Undo、端点起点分岐のプレビュー一致・本線形状・切替・両経路走行・カント解除・Undo。
- 実装コミット: `b2bf9db`
- 同期: `origin/main`へ同期済み。
