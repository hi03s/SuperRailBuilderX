...
- レール生成ツール `SuperRailBuilderX_builder1` の基本機能を実装済み。既設端の接続ブロックをSRB3と同じ所有ブロック基準へ修正し、既設端同士の滑らかな接続、既設端と自由点の正円近似、方向に応じた自由点所有ブロック、カーブ半径表示まで反映済み（実機再検証待ち）。
...
- builder1の今回のログでは生成成功が複数確認できた一方、破壊的敷設中に2回 `builder roadbed tile missing` が発生した。接続先所有ブロックの誤りは修正したが、再発時に特定できるよう設置前後のブロック・metadata・TileEntityと生成計画を診断出力する。生成・走行・保存の再確認が必要。
...
3. 自由点が視線位置より1/16ブロック上になること、PのONで始点が0.5ブロック格子、終点が始点基準の0.5 m距離になること、Ctrl+Pの1度・5度・15度で終点方向が変わることを確認する。
4. `selectCursor` / `selectedCursor`は精密点、`selectCursorMarker` / marker0〜7はマーカーブロック中央へ表示されることを確認する。特に既設端では見かけの精密端点ではなく元RailPositionのブロックとdirectionから求めた接続先になり、元レールを上書きしないことを確認する。
5. 自由点をブロックの縁・角に置き、レールが進入する側の隣接ブロックがmarker所有位置として選ばれ、marker0〜7の向きもレール方向と一致することを確認する。
6. 既設レール端部同士を接続し、SRB3方式の曲線長由来アンカーによるプレビューと生成結果が滑らかにつながり、接続する3本を走行できるか確認する。
7. 既設端部と空間点を両方の選択順で接続し、正円近似カーブ、カーブ半径パネル、生成結果が一致して接続レールを走行できるか確認する。
8. 通常ブロック、別レール道床、別レールコアを横切る生成を試し、強制置換後の新設レール走行と既設側への影響を確認する。
9. Ctrl+Zで直前の生成レールだけが撤去され、生成時の選択が戻り、破壊済みブロックは戻らないことを確認する。
10. 結果と `latest.log` を共有する。`builder creation plan`、`builder roadbed tile missing`、例外、`Rail not found` を重点確認する。
11. builder1確認後、従来のRailPositionテストについて `docs/CODEX_HANDOFF.md` 2026-09-01記録の空気限定道床・通常レール再生成項目を継続確認する。
...
- 2026-09-02 ローカルCodex:
  確認してほしいこと: builder1の接続先所有ブロック、自由点の境界における所有ブロック、既設端同士の滑らかな曲線、既設端・自由点間の正円近似と半径表示、走行、破壊的交差、専用Undo。
  確認方法: 必ずバックアップ済みテストワールドを使用し、上記「次に行うこと」1〜10と `docs/builder1.md` の実機検証を確認する。
