...
- builder1の-X/-Z直線だけ道床片側が凹凸になる問題に対し、道床幅計算の三角関数誤差を正規化。勾配指定は正円近似縦曲線＋目標勾配直線へ分割し、距離不足時は到達可能な正円近似だけを生成する方式へ変更済み（実機検証待ち）。
...
- builder1の勾配指定は、既設端側の正円近似縦曲線と目標勾配の直線を別論理レールとして生成する。距離不足時の正円近似のみ、内部接続点の走行、2本一括Undo、選択順が逆の場合、-X/-Z道床対称化を実機確認する必要がある。
...
8. ↑/↓で自由候補が1 m、Ctrl+↑/↓で1/16 m上下し、長押しできること、Fで高さだけ0へ戻ることを確認する。勾配値・曲線半径変更がチャットへ連続表示されないことも確認する。
9. 既設端を始点に選ぶたび、自由終点が始点勾配の延長線上になり初期終端勾配も一致すること、↑/↓で1‰ずつ変わることを確認する。自由点→既設端では自由始点側が変わることを確認する。
10. 両端の`slopePanel`表示とプレビュー、正円近似縦曲線＋直線勾配の分割生成、距離不足時の正円近似のみの生成を確認する。内部接続点を両方向に走行し、再入場後保存、2本一括Undo、低いY側からの生成順を確認する。
...
- 2026-09-03 ローカルCodex:
  確認してほしいこと: builder1の-X/-Z直線における道床対称化、勾配の正円近似＋直線分割、距離不足時の正円近似、内部接続の走行、2本一括Undo。
  確認方法: 必ずバックアップ済みテストワールドを使用し、上記「次に行うこと」8〜13と `docs/builder1.md` の実機検証を確認する。
