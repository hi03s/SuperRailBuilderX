...
- 担当: ローカルCodex
  開始日: 2026-09-06
  内容: 再現ログを解析し、builder1のチャンク境界交差、複線コピーの段階操作・Undo復元・生成不能・削除後同期、線路分割の透明表示を修正する。
  主な変更予定: `SRBXApiCompat.ts`、複線コピー/線路分割クライアントスクリプト、モデル描画、診断ログ、関連資料
...
- 分割・複製・builder1生成レールで、低速時だけ次の論理RailMapへ遷移できず押し戻される場合がある。速度は0にならず`Rail not found`もないため原因未確定。
- 生成時に`[SuperRailBuilderX transition]`へ両端の所有ブロック・接続先所有コアを出す診断を追加した。問題発生方向・速度と該当ログを対応させて再確認する。

### 複線コピーツール

- 基本操作・Undo・概ねの走行は実機確認済み。強調表示の座標補正と、Undo後に生成時の選択へ戻す処理は再確認待ち。
- ログ上の生成不能は、連続複製の途中で安全なセクションコア位置がなくなった`section_core_conflict`。既存レールを壊さない安全停止とロールバックは機能している。失敗計画とロールバック結果の診断を追加済み。
- 実生成はKaizPatchX向け。分岐器は対象外で、mc1710・mc1122の生成処理は既存builder1互換層に従って安全に停止する。

### 線路分割ツール

- 基本操作・Undo・概ねの走行は実機確認済み。強調表示の座標補正、分割元モデルへのフォールバック、`distancePanel`表示、片側2 m以下の候補除外は再確認待ち。
...
- builder1の実機確認項目、Undo、概ねの走行は正常確認済み。低速遷移の共通問題と、道床が重なるレール交差は確認が残っている。
...
1. バックアップ済みワールドで、両ツールの黄色/水色強調表示を再確認する。
2. 複線生成後に別レールを選び、Ctrl+Zで生成時の選択だけが復元されることを確認する。
3. レール非所持時の分割元モデル、`distancePanel`、片側2 m以下の候補除外を確認する。
4. 低速遷移または生成・Undo失敗時は、条件と`[SuperRailBuilderX transition]`、`[SuperRailBuilderX double-track-copy]`、`undo rail`周辺のログを共有する。
