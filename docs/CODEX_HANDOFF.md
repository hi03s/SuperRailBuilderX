...
- AppleExtended実験対応をmainへ統合済み。通常レール端点移動・同期・Undoに加え、SRBX側の一時compatでレール生成Aと複線コピーの通常レール生成・Undoを有効化した。自動分割依存機能は安全に無効化する。
...
2. AE環境で通常レール端点移動に加え、レール生成A・複線コピーの通常レール生成、接続、モデル継承、走行、Undoを確認する。
