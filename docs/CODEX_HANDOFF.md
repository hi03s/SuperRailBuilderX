...
- 最新`main`で`pnpm gen`と`pnpm build`が成功し、全3ターゲットの生成・ビルドを確認済み。
- 2026-09-06の実機確認を反映し、両ツールの強調表示、複線Undo時の選択復元、分割元モデル継承、距離パネル、分割最小長を修正。全3ターゲットの生成・ビルド確認済み。
- builder1のチャンク境界付近のセクションコア交差生成・走行、複線コピーの位置固定操作・Undo選択復元、分割パネルと縦勾配精度は実機確認済み。2026-09-06の追加修正は実機再確認待ち。
...
- 分割・複製・builder1生成レールで、低速時だけ次の論理RailMapへ遷移できず押し戻される問題を再確認。今回のログだけでは発生時刻・方向と生成ログを対応できず、原因は未確定。
- KaizPatchXの遷移処理は端点を越えたと判定した場合だけ接続先探索へ進むため、低速時の座標拘束との相互作用が候補。問題発生方向・おおよその時刻・速度と`[SuperRailBuilderX transition]`を対応させて再確認する。

### 複線コピーツール

- 基本操作・強調表示・位置固定→Enter生成・Undo選択復元・概ねの走行は実機確認済み。
- Undo復元後の左クリック履歴を再構成した。曲線内側は近似半径の中心へ達する前に列生成を打ち切り、反転線形とUndoゴーストの原因になった元線側への回り込みを防止した。両方とも実機再確認待ち。
- 実生成はKaizPatchX向け。分岐器は対象外で、mc1710・mc1122の生成処理は既存builder1互換層に従って安全に停止する。

### 線路分割ツール

- 基本操作・Undo・概ねの走行、強調表示、分割元モデル、`distancePanel`、片側2 m以下の除外、透明表示、縦勾配精度は実機確認済み。
- 短い前後区間を単一通常レールとして生成して内部セクションコア同士の衝突を防ぎ、元レール両端の接続先も保護対象へ引き継いだ。カント付き分割点では`getRailHeight`に含まれるカント持ち上がり量を除去した。実機再確認待ち。
...
- builder1の基本機能、Undo、概ねの走行と、チャンク境界付近を含むセクションコア交差生成・走行は正常確認済み。通常交差生成時のチャット表示、64 m超の拒否、スナップON時の`selectCursorMarker`常時表示は実機確認待ち。
- builder1は破壊的な道床生成を行う。通常道床交差、内部セクションコア移設、既設コア衝突時の安全停止をバックアップ環境で継続確認する。
- builder1規則へ統合したRailPosition移動は、通常/自動分割、交差、接続先保護、信号・サブレール保持、失敗時復元、走行の実機確認が必要。
...
1. 複線コピーをUndoして選択復元した直後、左クリックで最後の選択を解除できることを確認する。曲線内側では曲率中心を越える列が表示・生成されず、Undoゴーストが出ないことも確認する。
2. 線路分割で以前`section_core_conflict`になった短い前後区間、既設レールへ接続した端部、カント付きレールを再確認する。
3. builder1の通常交差生成チャット、64 m超の拒否、スナップON時の`selectCursorMarker`を確認する。
4. RailPosition移動で通常/自動分割、既設コア交差、信号・サブレール保持、生成失敗時復元と走行をバックアップ済みワールドで確認する。
5. 低速遷移を再現したら進行方向・速度・おおよその発生時刻を控え、`[SuperRailBuilderX transition]`を含むログを共有する。
