...
- builder1の半径固定、空中高さ、終端勾配、勾配パネル、低い側起点生成は実機操作されたが、固定半径の自由始点YawがプレイヤーYawと逆符号だったこと、垂直アンカー長0で勾配が直線補間されたことが判明した。Yaw符号と垂直アンカーを修正したため、縦曲線と操作方向の再確認が必要。
...
8. ↑/↓で自由候補が1 m、Ctrl+↑/↓で1/16 m上下し、長押しできること、Fで高さだけ0へ戻ること、変更値がチャットへ表示されることを確認する。
9. 既設端を始点に選ぶたび、自由終点が始点勾配の延長線上になり初期終端勾配も一致すること、↑/↓で1‰ずつ変わることを確認する。自由点→既設端では自由始点側が変わることを確認する。
10. 両端の`slopePanel`表示、プレビューと生成結果、直線補間ではない縦曲線、両方向走行・再入場後保存を確認する。`creation plan`でLengthHとLengthVが同値で、選択順と逆でも低いY側が起点になることを確認する。
...
12. 結果と`logs/latest.log`を共有する。`creation plan`、`generation order`、`protected connection`、`section core relocated`、`rail_core_conflict`、`roadbed/core tile missing`、`Rail not found`を重点確認する。
