...
- 他レールの非コア道床をRTM標準と同様に重ねられるよう衝突判定を修正し、交差・2本同時移動の回帰を修正済み（実機再検証待ち）。
...
1. バックアップ済みテストワールドで、以前成功していた見かけ上のレール重ね・交差を再試行し、非コア道床の重なりで停止しないことを確認する。
2. 同じ端点を持つ2本を同時移動し、`partial_target_1:roadbed_conflict` が再発せず両方とも移動することを確認する。
3. 通常レールを旧道床範囲外へ動かし、列車が `Rail not found` で停止しないことを確認する。レールコアや非レールブロックとの真の衝突は引き続き `roadbed_conflict` になる。
...
- 2026-09-01 ローカルCodex:
  確認してほしいこと: 見かけ上のレール重ね・交差と接続端点2本の同時移動が復旧し、不足道床追加後に範囲外走行できること。
  確認方法: 必ずバックアップ済みテストワールドを使用する。上記「次に行うこと」の順に確認し、`allowing overlapping foreign roadbed`、`partial_target`、`Rail not found` の有無を共有する。
