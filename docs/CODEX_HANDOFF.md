...
- builder1の-X/-Z直線だけ起点側道床が凹型になる問題に対し、道床幅計算の三角関数誤差の正規化に加え、端部隣接座標に属する幅サンプル全体を除外するよう修正済み（実機検証待ち）。
- builder1の勾配指定は、初期値1000 mの設定可能な縦曲線半径で正円近似縦曲線＋目標勾配直線へ分割し、距離不足時は到達可能な正円近似だけを生成する方式へ再修正済み。Ctrl+↑/↓で1000 m刻みに変更し、値をチャット表示する（実機検証待ち）。
...
- builder1の勾配指定は、既設端側の正円近似縦曲線と目標勾配の直線を別論理レールとして生成する。指定縦曲線半径、距離不足時の正円近似のみ、内部接続点の走行、2本一括Undo、選択順が逆の場合、-X/-Z起点側道床対称化を実機確認する必要がある。
...
9. 既設端を始点に選ぶたび、自由終点が始点勾配の延長線上になり初期終端勾配も一致すること、↑/↓で目標勾配が1‰ずつ変わることを確認する。Ctrl+↑/↓では縦曲線半径が初期1000 mから1000 m刻みで変わり、変更値がチャット表示されることも確認する。自由点→既設端では自由始点側が変わることを確認する。
10. 両端の`slopePanel`表示と安定したプレビュー、指定半径の正円近似縦曲線＋直線勾配の分割生成、分割点の`cantLine`、距離不足時の正円近似のみの生成を確認する。内部接続点を両方向に走行し、再入場後保存、2本一括Undo、低いY側からの生成順を確認する。
...
12. +X/+Zと-X/-Zへ同条件の直線を生成し、道床幅が左右対称で片側の凹み・張り出しがないことを確認する。
13. 結果と`logs/latest.log`を共有する。`vertical profile plan`、`creation plan`、`generation order`、`protected connection`、`section core relocated`、`rail_core_conflict`、`roadbed/core tile missing`、`Rail not found`を重点確認する。
...
- 2026-09-03 ローカルCodex:
  確認してほしいこと: builder1の-X/-Z直線における起点側道床対称化、設定可能な縦曲線半径による正円近似＋直線分割、距離不足時の正円近似、分割点`cantLine`、内部接続の走行、2本一括Undo。
  確認方法: 必ずバックアップ済みテストワールドを使用する。既設端を選択して↑/↓で目標勾配、Ctrl+↑/↓で縦曲線半径を変更し、チャット値とプレビューを確認する。20‰→0‰・半径1000 mでは約20 mで水平へ移ることを目安に、上記「次に行うこと」8〜13と `docs/builder1.md` の実機検証を確認する。
