...
- レール生成ツール `SuperRailBuilderX_builder1` に、既設コア交差の二段階置換、自由点候補のmarker非表示、Ctrl+C完全リセット、曲線半径固定・∞表示、空中高さ・終端勾配操作、両端勾配表示、低い側を生成起点にする処理まで実装済み（実機再検証待ち）。
...
- builder1の最新ログでは生成成功が多数ある一方、既設`BlockLargeRailCore`を道床へ直接置換した場合に、旧コア破壊処理の後発実行で新設道床が空気化する失敗を3回確認した。衝突コアを先に空気化して標準破壊処理を完了後に道床を置く二段階方式へ変更したため、単体失敗・交差失敗の再確認が必要。
...
- builder1の半径固定、空中高さ、終端勾配、勾配パネル、低い側起点生成はローカルの計算・ビルド検証のみ。特に小半径で長い弧長を指定した操作感、自由始点で半径設定を継続利用する際の始点向き、勾配付き自動分割レールの走行を実機確認する必要がある。
- builder1のボタンテクスチャは、指示どおり未配置の `textures/superrailbuilderx/button_builder1.png` を仮指定している。

## 次に行うこと

1. 必ずバックアップ済みテストワールドで `SuperRailBuilderX_builder1` を使用し、前回単体失敗した位置と既設レールコアを横切る交差を再現する。`intersecting core cleared before placement`の後に生成が成功するか確認する。
2. 接続対象自身のコアへ曲線が戻るケースが`connection_core_conflict`で元レールを壊さず停止し、その他の交差相手は従来仕様どおり結果的に破壊され得ることを確認する。
3. 自由点候補では`selectCursor`だけ、既設端部候補では`selectCursor`と`selectCursorMarker`が正しい接続ブロックへ表示されることを確認する。選択後のmarker0〜7は自由点にも表示される。
4. Cが選択だけを解除して設定を維持し、Ctrl+Cがスナップ、空中高さ、半径固定、勾配、専用Undoを初期化することを確認する。
5. 未選択でOを押すと∞、1点以上でOを押すと表示中の半径が固定されることを確認する。←/→は1 m、Ctrl+←/→は100 m、範囲は1〜10000 mであることを確認する。
...
8. ↑/↓で自由候補が1 m、Ctrl+↑/↓で1/16 m上下し、変更値がチャットへ表示されることを確認する。
9. 既設端を始点に選ぶたび、自由終点が始点勾配の延長線上になり初期終端勾配も一致すること、↑/↓で1‰ずつ変わることを確認する。自由点→既設端では自由始点側が変わることを確認する。
10. 両端の`slopePanel`表示、プレビューと生成結果、勾配付きレールの両方向走行・再入場後保存を確認する。選択順と逆でも低いY側が生成起点になることを`generation order`ログで確認する。
11. 通常ブロック・道床・コアを横切る短距離・複数チャンク生成、Ctrl+Z、手持ちモデル、即時描画を再確認する。
12. 結果と`logs/latest.log`を共有する。`creation plan`、`generation order`、`intersecting core cleared`、`connection_core_conflict`、`roadbed/core tile missing`、`Rail not found`を重点確認する。
...
- 2026-09-02 ローカルCodex:
  確認してほしいこと: builder1の既設コア二段階置換、候補marker表示、完全リセット、半径固定、空中高さ、終端勾配、勾配パネル、低い側起点生成、走行と保存。
  確認方法: 必ずバックアップ済みテストワールドを使用し、上記「次に行うこと」1〜12と `docs/builder1.md` の実機検証を確認する。交差相手は破壊され得るため、試験用レールだけを使用する。
