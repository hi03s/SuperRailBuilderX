...
- レールを再び選択でき、平行移動・端点移動・Undo後も前後の接続レールを含むホバー強調が現在の線形で表示されることを確認する。

### カント整形

- レール端点・中央・任意位置を再び選択できることを確認する。
- 曲線/直線の共有端点をどちらのレール側から選択しても曲線側の半径・カントが使われ、接続両側へ同じ物理方向で適用されることを確認する。不一致時は`[SuperRailBuilderX cant] selected:`を含むログを共有する。
- レール中央を複数回選択し、曲線の左右に対してカント方向が安定することを確認する。

### 分岐生成

- 共有端点の選択前は両レールが黄色、選択後は分岐先カーソルを伸ばした側だけ水色となり、分岐先確定後は非ベース側の黄色が消えることを確認する。
- 分岐化後に新しく接する既設レールもカント0となり、Undoで元へ戻ることを確認する。
- 端点・中央の分岐生成直後にクライアント例外が出ず、Undoで元レールへ戻ることを確認する。
...
- 2026-09-13 ローカルCodex: KaizPatchXで空だったロード済みTE一覧をチャンク内コア列挙で補い、レール移動・カント選択を復旧。分岐生成待機中のnull RailPosition参照を防ぎ、分岐先確定後は非ベース側強調を消すよう修正。詳細は月別履歴とコミット`b78ab58`を参照。
