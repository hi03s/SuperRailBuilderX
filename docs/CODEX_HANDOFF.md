...
- レール生成ツール `SuperRailBuilderX_builder1` の基本機能を実装済み。初回実機ログで判明した道床設置の誤判定を修正し、自由点の1/16高、RTM方向規約、マーカーブロック位置、始点基準の距離・角度スナップ、3次ベジェプレビューを反映済み（実機再検証待ち）。
...
- builder1の初回実機試験では自由点・既設端点とも最初の道床で`create_failed`となった。`World#setBlock`のfalseを即失敗扱いせず設置後TileEntityで判定するよう修正したため、生成・走行・保存の再確認が必要。
...
4. `selectCursor` / `selectedCursor`は精密点、`selectCursorMarker` / marker0〜7はマーカーブロック中央へ表示され、自由点markerの向きがレール方向と一致することを確認する。
5. 既設レール端部同士を接続し、プレビューが直線ではなく両端アンカーに沿う3次ベジェとなり、精密位置、反転direction、アンカー角・勾配、長さ2/3で滑らかにつながり、接続する3本を走行できるか確認する。
6. 既設端部と空間点のEnter生成が安全に拒否されることを確認する。
7. 通常ブロック、別レール道床、別レールコアを横切る生成を試し、強制置換後の新設レール走行と既設側への影響を確認する。
8. Ctrl+Zで直前の生成レールだけが撤去され、生成時の選択が戻り、破壊済みブロックは戻らないことを確認する。
9. 結果と `latest.log` を共有する。`[SuperRailBuilderX builder1]`、例外、`Rail not found` を重点確認する。
10. builder1確認後、従来のRailPositionテストについて `docs/CODEX_HANDOFF.md` 2026-09-01記録の空気限定道床・通常レール再生成項目を継続確認する。
...
- 2026-09-02 ローカルCodex:
  確認してほしいこと: builder1の生成失敗解消、自由点の高さ・方向、精密カーソルとマーカーブロック位置、始点基準スナップ、既設端部間のベジェプレビュー、走行、破壊的交差、専用Undo。
  確認方法: 必ずバックアップ済みテストワールドを使用し、上記「次に行うこと」1〜9と `docs/builder1.md` の実機検証を確認する。
