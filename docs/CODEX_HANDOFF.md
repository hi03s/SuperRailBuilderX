...
- `SuperRailBuilderX_builder1`を実装済み。JSON識別名はbuilder1を維持し、文書・ヘルプでは`レール生成A`と表記する。自由点・既設端接続、曲線半径固定、勾配・縦曲線、複数レール一括Undo、道床・コア保護を備える。
...
- `SuperRailBuilderX_CantFormatter`を実装済み。自動分割コアは公開Section NBT APIで更新し、5種類の軌間名と最大カントをチャット・ヘルプへ表示する。
- `SuperRailBuilderX_BranchBuilder`を実装済み。単純分岐の根元は分割元半区間のアンカーを保持し、半径パネルの透明描画、約0.5 m分割候補、Undoに対応する。
...
- カント整形を通常/自動分割レールへ適用し、`writeToNBT`例外が再発せず、5種類の軌間名・最大カント、複数適用・Undo、左右曲線の符号が正しいことを確認する。
- 分岐生成で根元から中央までの本線描画、半径パネルの透明背景、自由点/別レール端点、モデル、分岐切替、両経路走行、道床、Undoを確認する。
- 勾配・縦曲線・既存分岐・在線・短レールが安全に拒否されることを確認する。
- 分割・分岐の約0.5 m候補がレールパーツ描画位置と一致し、生成後の接続点を低速で双方向に通過できることを重点確認する。

## 次に行うこと

1. 修正したカント整形・分岐生成・分割位置を各資料に従ってバックアップ済みワールドで再確認する。
