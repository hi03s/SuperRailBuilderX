...
- 正式版`SuperRailBuilderX_RailMover`は通常・自動分割レールとも元状態を退避し、builder1と同じ衝突判定・道床生成規則で再生成する。論理RailMapの複数選択・一括平行移動・一括Undo、Ctrl押下中の追加選択抑止、Undo後のRailMap再同期に対応する。
...
- `SuperRailBuilderX_CantFormatter`を実装済み。端点・中央・端部/中央から10 m以上離れた任意点を選択でき、任意点は分割して適用する。変更対象の水色/黄色強調、共有端点の連続適用、分割を含むUndoに対応する。
- `SuperRailBuilderX_BranchBuilder`を実装済み。中央の約0.5 m候補、接続/未接続の正確な端点を根元とする単純分岐、カーソル側のベース選択、分岐化後の接続部カント0化とUndoに対応する。
...
- Undo後、非選択の接続レールへホバーした強調形状が移動前の線形へ戻ることを確認する。
- Ctrlを押したまま未選択レールを右クリックしても追加されず、選択済みレールは右クリックで解除できることを確認する。
- 初期化されていない自動分割コアのクラッシュ防止は確認方法がないため省略する。再現できた場合だけ`getLogicalRailPositions(...) must not be null`の有無を確認する。
...
- カント整形で曲線/直線の共有端点を曲線側から選び、直線の0半径判定に吸われないことを確認する。中央選択は水色、端部・中央から10 m以上離れた任意点は黄色になり、後者は選択位置で分割・適用されて1回のUndoで元へ戻ることも確認する。
- 分岐生成で未接続端点を選択できること、共有端点ではカーソルを向けた側へベースレールが切り替わること、分岐化後に新しく接する既設レールもカント0となりUndoで戻ることを確認する。
...
1. 修正したレール移動、カント整形、分岐生成を各資料に従ってバックアップ済みワールドで再確認する。
