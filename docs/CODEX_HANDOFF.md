...
- 12〜20 m程度の通常レールを、分割後の両側が6 mを超える複数位置で分割する。前半終端の道床と後半コアが同一ブロックになる条件でも失敗・二重表示せず、両側を走行でき、Ctrl+Zで元の1本へ復元されることを確認する。

### builder1

- 既設レール端部のホバー時だけ、自由点用`selectCursorMarker`ではなく接続用`snapCursorMarker`が表示されることを確認する。
- 手にレールを持たず既設端点から勾配・縦曲線を生成し、内部で2本へ分割されても両方が接続先のモデル・テクスチャになることを確認する。
...
- 通常の端点移動、単体平行移動、前後端点を伴う平行移動をそれぞれCtrl+Zで取り消し、レール・接続端点・描画・走行が移動前へ戻ることを確認する。
