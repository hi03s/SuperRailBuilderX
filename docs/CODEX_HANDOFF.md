...
- 前後に複製済みレールがある連続レールを複数選択して生成し、通常道床と重なる内部セクションコアを含めても全計画が生成されること、失敗時は途中生成分だけが安全にロールバックされることを確認する。
...
- スナップON時、`selectCursorMarker`の3×3表示が丸めた選択座標ではなく実際の視線衝突位置へ追従することを確認する。
- 既設端点と地上自由点を勾配状態で選択して自由点側を0‰にし、Iで自由点がブロック上1/16 mへ移動すること、既設側の直線勾配と終端側の縦曲線として生成・走行できることを確認する。

### レール移動ツール

- モデル一覧と操作表示が`レール移動ツール`へ変わっていることを確認する。
- 重なる2本の端部を互い違いにし、経路途中の通常道床は既設側、移動後のセクションコア位置と非コア終端道床は新設側が所有して、両線を双方向に走行できることを確認する。
