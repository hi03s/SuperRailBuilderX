...
- Web側ではGradle 9.4.1をネットワーク制限により取得できず、`pnpm gen`・`pnpm build`は未完了。ローカルCodexで最初に実施する。
...
1. ローカルCodexで最新`main`を取得し、`pnpm gen`と`pnpm build`を実行する。
2. 型またはビルドエラーがあれば、線路分割ツールの変更ファイルに限定して修正する。
3. ビルド成功後、`docs/rail-splitter.md`の実機確認項目をバックアップ済みワールドで実施する。
4. 問題時は、分割前のレール条件・選択位置・手持ちモデルと、`[SuperRailBuilderX splitter]`周辺の必要最小限のログを共有する。
...
- 2026-09-03 Web側Codex: 引継ぎ帳を短期情報と月別履歴へ分離し、`AGENTS.md`へコンテキスト・トークン使用量の管理規則を追加。
