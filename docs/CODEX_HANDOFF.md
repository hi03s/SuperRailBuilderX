...
- `SuperRailBuilderX-alpha-0.1.0.zip`を導入し、4ツールの本体・ボタン・アイコンが更新後のテクスチャで表示され、同梱readme.txtを開けることを確認する。
...
- 全長6 m以下のレールが赤く強調され、選択できないことを確認する。8〜20 m程度のレールは両側が3 m超になる候補で分割でき、3 m以下になる端寄りの位置を選択できないことを確認する。

### レール移動ツール

- 前後にレールが接続された1本を平行移動し、前後レールの接続端点も新しい両端へ連動して移動することを確認する。
- 移動先の片側だけに接続先がある場合は反対の自由端も同じ補正量だけ動いて元形状を保ち、両側に接続先がある場合は両端が正確に接続されることを確認する。
- 複数の論理レールを右クリックで選択・解除し、同じ平行距離でまとめて移動できることを確認する。
- 通常の端点移動、単体・複数平行移動をそれぞれCtrl+Zで取り消し、チャット応答があり、レール・連動端点・描画・走行が移動前へ戻ることを確認する。
- 端点移動・平行移動・Undo直後に、端部付近の一部モデルが180度反転せず、ホバー強調が新しい形状と一致することを確認する。再入場後も同じ形状であることを確認する。
