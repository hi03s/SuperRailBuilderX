...
- `SuperRailBuilderX_CantFormatter`を実装済み。自動分割コアはSection NBTを実行時MCP名で更新し、5種類の軌間名と最大カントをチャット・ヘルプへ表示する。
- `SuperRailBuilderX_BranchBuilder`を実装済み。中央の約0.5 m候補からの単純分岐、正確な端点からの通常レール、接続部カントの0化とUndoに対応する。
...
- カント整形を通常/自動分割レールへ適用し、`setTag`例外が再発せず、5種類の軌間名・最大カント、複数適用・Undo、左右曲線の符号が正しいことを確認する。
- 分岐生成で中央候補からの分岐と正確な端点からの通常レール、モデル、分岐切替、両経路走行、道床、接続部カント0化・Undoを確認する。
