...
通常ブロックはbuilder1と同じく道床へ置換します。既設レールコアは保護し、セクションコアを安全な位置へ配置できない場合は、長さ64 m以下に限って移動側を単一通常レールにします。移動専用規則では移動後レールの走行経路を所有させるため重複通常道床を置き換えますが、別レールの両端から約2 m以内の道床は接続遷移を維持するため保護します。
...
SuperRailBuilderXはbuilder1と共通の再生成経路を使い、元RailPropertyの`autoSplit`と新しい曲線からセクション数を再計算します。`RailChunkSectioner.split()`の結果から各セクションを直接構成し、既設コアを保護しながら必要な道床を生成します。経路上の未保護コアがKaizPatchXセクションコアだけの場合、または内部セクションコアの安全な代替位置がない場合は、長さ64 m以下に限って移動側を単一通常レールへ切り替えます。
...
2026-09-06の実機確認では、builder1・線路分割・複線コピーで生成した接続レールについて、低速時だけ次の論理RailMapへ遷移せず押し戻されるように見える場合があった。速度は0にならず、`Rail not found`も記録されないため、単純な道床欠落とは断定できない。

生成後に`[SuperRailBuilderX transition]`として、論理レールキー、コア座標、両端の精密座標・所有ブロック、接続先ブロックと実際の所有コアを記録する。再検証では問題が起きた進行方向・速度・接続位置とこの行を対応させ、次を切り分ける。

- 2本の精密端点座標と方向が一致しているか。
- 接続元・接続先の道床TileEntityが想定した論理コアを所有しているか。
- 理論端点より先に別レール所有の道床があり、低速時のRailMap探索順へ影響していないか。
- RTMまたはKaizPatchX標準生成レールでも同じ条件で再現するか。
...
builder1とレール移動ツールは、通常ブロックを道床へ置換しますが、経路途中で既設レール所有の通常道床と重なる場合は既設側の所有情報を維持します。例外として、新設レールの生成起点ではない非コア終端道床は端部遷移のため新設側へ置き換え、新しいセクションコア予定位置は既設通常道床をコアへ置換します。セクションコア予定位置が既設コアと重なる場合だけ、同一チャンク・区間内の空気・マーカー・通常道床へ移設します。この所有規則は、重なる線路の端部を互い違いに配置して双方の遷移用道床を確保することを前提とします。
...
| 内容                               | ファイル                                                       |
| ---------------------------------- | -------------------------------------------------------------- |
| クライアント操作・候補探索・描画   | `render_rail_mover.ts`                                         |
| レール移動サーバー                 | `server_rail_mover.ts`                                         |
