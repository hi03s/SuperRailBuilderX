...
builder1は上記の非破壊な移動試験とは異なり、クリエイティブ敷設として他レールの破壊を許容します。ただし既設コアを道床で直接上書きすると、旧コアの`breakBlock`側が論理レールを撤去し、新設直後の道床まで空気化することがあります。経路上のコアを事前収集・在線確認し、衝突コアブロックを先に空気化して標準破壊処理を完了させてから新しい道床を配置する順序が必要です。builder1自身は論理レール撤去APIを直接呼びません。接続対象として選んだ論理レールは事前空気化の対象外とし、そのコア自体へ経路が交差する場合は安全に停止します。
