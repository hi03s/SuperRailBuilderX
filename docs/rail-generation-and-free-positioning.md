...
SRB3の実装では、精密なレール端点とマーカーブロックの表示位置を分離しています。方向マーカーは`blockX + 0.5`、`blockZ + 0.5`を基準に描画し、既設端部へ接続するときは元RailPositionのdirectionから求めた隣接ブロックを使用します。精密端点のY座標には通常レール高の1/16ブロックを加えます。
