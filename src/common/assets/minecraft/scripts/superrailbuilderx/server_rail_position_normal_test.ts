...
function applyRequest(
	...
	NGTLog.debug(
		`[SuperRailBuilderX RailPosition Normal] applying connected endpoint: targets=${resolved.length}`,
	);
	for (let i = 0; i < resolved.length; i++) {
		...
		if (
			...
			NGTLog.debug(
				`[SuperRailBuilderX RailPosition Normal] connected endpoint apply failed: target=${i}, applied=${i}, result=${result}`,
			);
			return i > 0
...
function onUpdate(entity: EntityVehicle, scriptExecuter: ScriptExecuter): void {
	...
	if (request) {
		try {
			...
			ErrorLogger.log(
				"SuperRailBuilderX RailPosition Normal apply",
				"applyRequest",
				error,
